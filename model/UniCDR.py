...
class UniCDR(nn.Module):
    ...
    def forward_user(self, domain_id, user, context_item, context_score, global_item, global_score):
        ...
        if self.training and (self.warmup == 0):
            device = share_user.device  # Get the device of share_user
            random_label = (torch.arange(0, share_user.size(0), 1).to(device) + torch.randint(1, share_user.size(0), (1,), device=device).item()) % share_user.size(0)
